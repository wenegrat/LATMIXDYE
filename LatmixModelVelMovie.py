...
for i in range(0, nx):
    ...
    bi[:,:,i] = f(timesi)
    f = interpolate.interp1d(time_f/86400+64.5-64.87, u_f[0:686,:,i], axis=0)
    ui[:,:,i] = f(timesi)
#%%
uzi = np.gradient(ui,axis=1)/np.gradient(z[np.newaxis,:,np.newaxis], axis=1)
#%%
...
contsu = np.linspace(-1e-1, 1e-1, 100)
contsu = np.linspace(-2e-2, 2e-2, 100)
...
#    bvar = b_f[i,:-1,:]
    bvar = bi[i,:-1,:]
#    uvar = ui[i,:-1,:] - np.mean(ui[i,:-1,:])
    uvar = uzi[i,:-1,:]
    im = axdye.contourf(x/1e3, z[:-1], uvar, contsu, extend='both', cmap = cmap)

    for c in im.collections:
        c.set_edgecolor("face")
    axdye.contour(x/1e3, z[:-1], bvar, bconts, colors='0.7',  linewidths=0.5)
    axdye.contour(x/1e3, z[:-1], dyevar, conts, cmap = 'gnuplot')
...
#    axflux.text( -0.22, 0.75 ,'c)', color='k', size=20, bbox=dict(facecolor='w', edgecolor='k'),transform=axflux.transAxes )
    ...
    cb.set_ticks([contsu[0], 0, contsu[-1]])
    cb.set_label(r'$\frac{\partial u}{\partial z}$ [s$^{-1}$]')
    fig.tight_layout(pad=0.2)
    if save:
        plt.savefig(f'/home/jacob/Dropbox/GulfStreamDye/MovieFigsVel/LatmixVelModel_{i}.png')
        plt.close('all')
    
## TO MAKE MOVIE
# ffmpeg -start_number 0 -i LatmixVelModel_%d.png -pix_fmt yuv420p -crf 30 shear_model.mov
